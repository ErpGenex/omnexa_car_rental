...
def check_geofence(vehicle: str, latitude: float, longitude: float) -> dict:
	zones = frappe.get_all("Rental Geofence Zone", filters={"is_active": 1}, fields=["name", "center_latitude", "center_longitude", "radius_km"])
	inside = []
	for z in zones:
...
def analyze_driver_behavior(rental_driver: str) -> dict:
	tracks = frappe.db.count("Vehicle GPS Track", {"vehicle": ["in", frappe.get_all("Rental Contract", filters={"driver": rental_driver}, pluck="vehicle") or ["NONE"]]})
