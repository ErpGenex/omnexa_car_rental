...
def compute_dynamic_price(company: str, branch: str, vehicle_group: str | None = None, rental_type: str = "Daily") -> dict:
	...
	demand_factor = 1.0 + (frappe.db.count("Rental Booking", {"company": company, "branch": branch}) % 10) * 0.02
...
def fleet_copilot_query(query: str) -> dict:
	return {"query": query, "answer": f"Fleet insight: {frappe.db.count('Vehicle')} vehicles in fleet.", "agent": "fleet"}
