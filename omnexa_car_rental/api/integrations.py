...
def find_stations_nearby(latitude: float, longitude: float, radius_km: float = 25) -> list[dict]:
	stations = frappe.get_all("Rental Station", filters={"is_active": 1}, fields=["name", "station_name", "branch"], limit=50)
	for s in stations:
...
def sync_erp_intercompany(company: str) -> dict:
	return {"company": company, "synced_documents": frappe.db.count("Rental Contract", {"company": company}), "status": "OK"}
